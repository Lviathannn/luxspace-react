...
import useAsync from "../../helpers/hooks/useAsynch";
      ...
      .map((_, index) => {
         return (
            <div className="px-4 relative card group">
               <div className="rounded-xl overflow-hidden card-shadow relative w-[287px] h-[386px] bg-gray-200"></div>
...
export default function JustArrived() {
   const { data, run, isLoading, error } = useAsync();
                  ...
                  {data.data.map((item) => {
                     return (
                        <div className="px-4 relative card group" key={item.id}>
                           <div className="rounded-xl overflow-hidden card-shadow relative w-[287px] h-[386px]">
                              <div className="absolute opacity-0 group-hover:opacity-100 transition duration-200 flex items-center justify-center w-full h-full bg-black bg-opacity-50">
